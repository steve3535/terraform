...
# BEGIN ANSIBLE MANAGED BLOCK VSL-POC-MMO-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-POC-MMO-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-03.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_INT_DC1_01.id 
  firmware             = "efi"
  name                 = "VSL-POC-MMO-001" 
  folder               = ""
  num_cpus             = "2"
  memory               = "4096"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-poc-mmo-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.23.86.0/24","1") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.23.86.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
    }
  }        
  network_interface {
    network_id = data.vsphere_network.PRO_ANALYTICS_EXT.id
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.23.86.1/24 ipv4.gateway 172.23.86.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-poc-mmo-001,' -e env=DEV_TEST config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-POC-MMO-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-PRO-SQU-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-PRO-SQU-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-07.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_EXT_DC1_to_DC2.id 
  firmware             = "efi"
  name                 = "VSL-PRO-SQU-001" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-pro-squ-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.22.108.0/24","50") 
        ipv4_netmask = "24"
      }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.22.108.50/24 ipv4.gateway 172.22.108.1 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-pro-squ-001,' -e env=DEV_TEST config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-PRO-SQU-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-PRO-KCE-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-PRO-KCE-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-03.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_EXT_DC1_to_DC2.id 
  firmware             = "efi"
  name                 = "VSL-PRO-KCE-001" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-pro-kce-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.23.73.0/24","1") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.23.73.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
    }
  }        
  network_interface {
    network_id = data.vsphere_network.PRO_AUTH_EXT.id
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.23.73.1/24 ipv4.gateway 172.23.73.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-pro-kce-001,' -e env=PROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-PRO-KCE-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-PRO-KCE-002 (DMZ)
resource "vsphere_virtual_machine" "VSL-PRO-KCE-002" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-04.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_EXT_DC2_to_DC1.id 
  firmware             = "efi"
  name                 = "VSL-PRO-KCE-002" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-pro-kce-002"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.23.73.0/24","2") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.23.73.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.23.73.2/24 ipv4.gateway 172.23.73.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-pro-kce-002,' -e env=PROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-PRO-KCE-002 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-PRO-KCI-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-PRO-KCI-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-03.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_INT_DC1_to_DC2.id 
  firmware             = "efi"
  name                 = "VSL-PRO-KCI-001" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-pro-kci-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.23.42.0/24","1") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.23.42.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
    }
  }        
  network_interface {
    network_id = data.vsphere_network.PRO_AUTH_INT.id
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.23.42.1/24 ipv4.gateway 172.23.42.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-pro-kci-001,' -e env=PROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-PRO-KCI-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-PRO-KCI-002 (DMZ)
resource "vsphere_virtual_machine" "VSL-PRO-KCI-002" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-04.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_INT_DC2_to_DC1.id 
  firmware             = "efi"
  name                 = "VSL-PRO-KCI-002" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-pro-kci-002"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.23.42.0/24","2") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.23.42.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.23.42.2/24 ipv4.gateway 172.23.42.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-pro-kci-002,' -e env=PROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-PRO-KCI-002 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-REC-KCE-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-REC-KCE-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-03.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_EXT_DC1_01.id 
  firmware             = "efi"
  name                 = "VSL-REC-KCE-001" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-rec-kce-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.25.73.0/24","1") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.25.73.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
    }
  }        
  network_interface {
    network_id = data.vsphere_network.REC_AUTH_EXT.id
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.25.73.1/24 ipv4.gateway 172.25.73.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-rec-kce-001,' -e env=RECETTE_PREPROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-REC-KCE-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-REC-KCE-002 (DMZ)
resource "vsphere_virtual_machine" "VSL-REC-KCE-002" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-04.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_EXT_DC2_01.id 
  firmware             = "efi"
  name                 = "VSL-REC-KCE-002" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-rec-kce-002"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.25.73.0/24","2") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.25.73.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.25.73.2/24 ipv4.gateway 172.25.73.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-rec-kce-002,' -e env=RECETTE_PREPROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-REC-KCE-002 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-REC-KCI-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-REC-KCI-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-03.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_INT_DC1_01.id 
  firmware             = "efi"
  name                 = "VSL-REC-KCI-001" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-rec-kci-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.25.42.0/24","1") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.25.42.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
    }
  }        
  network_interface {
    network_id = data.vsphere_network.REC_AUTH_INT.id
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.25.42.1/24 ipv4.gateway 172.25.42.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-rec-kci-001,' -e env=RECETTE_PREPROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-REC-KCI-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-REC-KCI-002 (DMZ)
resource "vsphere_virtual_machine" "VSL-REC-KCI-002" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-04.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_INT_DC2_01.id 
  firmware             = "efi"
  name                 = "VSL-REC-KCI-002" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-rec-kci-002"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.25.42.0/24","2") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.25.42.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.25.42.2/24 ipv4.gateway 172.25.42.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-rec-kci-002,' -e env=RECETTE_PREPROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-REC-KCI-002 (DMZ)
