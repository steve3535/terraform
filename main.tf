...
# BEGIN ANSIBLE MANAGED BLOCK VSL-REC-WWW-001 (DMZ)
resource "vsphere_virtual_machine" "VSL-REC-WWW-001" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  ...
  network_interface {
    network_id = data.vsphere_network.REC_PUBLIC_SITE_EXT.id
  }
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-rec-www-001,' -e env=RECETTE_PREPROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-REC-WWW-001 (DMZ)
# BEGIN ANSIBLE MANAGED BLOCK VSL-TEST
resource "nutanix_virtual_machine" "VSL-TEST" {
        name                 = "VSL-TEST"
        description          = "VSL-TEST" 
        provider             = nutanix.dc3
        cluster_uuid         = data.nutanix_cluster.pe_lu651.metadata.uuid
        num_vcpus_per_socket = "1"
        num_sockets          = "1"
        ...
        boot_type            = "UEFI"
        nic_list {
          subnet_uuid = var.ahv_651_network["521-REC_ESIGN_INT"]
         }
        ...
        provisioner "local-exec" {
        command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsltest,' -e env=DEV_TEST config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file" 
        }
 }
# END ANSIBLE MANAGED BLOCK VSL-TEST
# BEGIN ANSIBLE MANAGED BLOCK VSL-TST-AXW-004
resource "nutanix_virtual_machine" "VSL-TST-AXW-004" {
        name                 = "VSL-TST-AXW-004"
        description          = "VSL-TST-AXW-004" 
        provider             = nutanix.dc3
        cluster_uuid         = data.nutanix_cluster.pe_lu651.metadata.uuid
        num_vcpus_per_socket = "1"
        num_sockets          = "1"
        ...
        guest_customization_cloud_init_user_data = base64encode(templatefile("user-data.yaml", {
          vm_domain         =  var.vm_domain 
          vm_name       =  "vsl-tst-axw-004"
          vm_ip   = "192.168.42.185"
          vm_prefix = "24"
          vm_gateway   =  "192.168.42.1"
          vm_dns1    = var.vm_dns1
          vm_dns2    = var.vm_dns2
        ...
        provisioner "local-exec" {
        command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-tst-axw-004,' -e env=DEV_TEST config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file" 
        }
 }
# END ANSIBLE MANAGED BLOCK VSL-TST-AXW-004
# BEGIN ANSIBLE MANAGED BLOCK VSL-TST-AXW-003 (DMZ)
resource "vsphere_virtual_machine" "VSL-TST-AXW-003" {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  host_system_id       = data.vsphere_host.nut-dmz-04.id 
  datastore_id         = data.vsphere_datastore.NUT_DMZ_INT_DC2_01.id 
  firmware             = "efi"
  name                 = "VSL-TST-AXW-003" 
  folder               = ""
  num_cpus             = "1"
  memory               = "2048"
  wait_for_guest_net_timeout = 5
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-tst-axw-003"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.26.83.0/24","2") 
        ipv4_netmask = "24"
      }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.26.83.2/24 ipv4.gateway 172.26.83.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-tst-axw-003,' -e env=DEV_TEST config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-TST-AXW-003 (DMZ)
