...
# BEGIN ANSIBLE MANAGED BLOCK VSL-PRO-WWW-001  (DMZ)
resource "vsphere_virtual_machine" "VSL-PRO-WWW-001 " {
  resource_pool_id     = data.vsphere_resource_pool.esx_pool.id
  ...
  clone {
    ...
    customize {
      linux_options {
      host_name = "vsl-pro-www-001"
      domain    = var.vm_domain
      }
    
    # Nécessaire malgré la config nmcli via le prov remote-exec car justement remote-exec a besoin d'une IP pour se connecter  
      network_interface {
        ipv4_address = cidrhost("172.23.87.0/24","1") 
        ipv4_netmask = "24"
      }
      ipv4_gateway = cidrhost("172.23.87.0/24","254")
      dns_server_list = [var.vm_dns1,var.vm_dns2]
    }
  }        
  network_interface {
    network_id = data.vsphere_network.PRO_PUBLIC_SITE_EXT.id
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "mkdir /home/localadmin/.ssh",
       ...
       "mv /tmp/authorized_keys /home/localadmin/.ssh/",
       "chmod 0600 /home/localadmin/.ssh/authorized_keys",      
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' ipv4.method manual ipv4.addresses 172.23.87.1/24 ipv4.gateway 172.23.87.254 connection.autoconnect yes",
       "sudo nmcli con mod 'System ${var.vsphere_interface_name}' con-name ${var.vsphere_interface_name}",
       "sudo nmcli con up ${var.vsphere_interface_name}",
       "sudo dnf -y remove cloud-init"      
    ]
  ...
  provisioner "local-exec" {
    command = " ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i 'vsl-pro-www-001,' -e env=PROD config.yml -u ${var.vm_user} -b --vault-password-file /opt/infrastructure/linux/vault/.vault_password_file"
  }

}
# END ANSIBLE MANAGED BLOCK VSL-PRO-WWW-001  (DMZ)
